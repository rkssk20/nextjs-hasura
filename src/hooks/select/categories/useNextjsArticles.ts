...
import { GET_NEXTJS_ARTICLES } from '@/graphql/queries'
import type { GetNextjsArticlesQuery } from '@/types/generated/graphql'
import { notificateState } from '@/lib/recoil'

const useNextjsArticles = () => {
  const [hasNextPage, setHasNextPage] = useState(true)
  const [cursor, setCursor] = useState(new Date().toJSON())
  const setNotificate = useSetRecoilState(notificateState)

  const {data, previousData, loading, networkStatus, fetchMore} = useQuery<GetNextjsArticlesQuery>(GET_NEXTJS_ARTICLES, {
    variables: {
      _eq: 1
    },
...
export default useNextjsArticles
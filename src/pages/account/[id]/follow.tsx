...
const Follow = ({ item, path }: AccountProps) => {
  const { data, networkStatus, hasNextPage, fetchMore, cursor } = usePersonFollows(path)
  ...
  return (
    ...
    >
      ...
      {/* 各アカウント */}
      {data && (data.person_follows.length > 0) ? data.person_follows.map((item, index) =>
          <Account
            ...
            setRef={index === (data.person_follows.length - 1) && setRef}
          />
        ) : (networkStatus === 7) && <Empty text='まだ誰もフォローしていません。' />
      }
